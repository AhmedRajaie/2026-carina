...
def run_tiktok_strategy(
    ...
    weekly_returns[lookback:] = (
        market.close[lookback:] / market.close[:-lookback] - 1.0
    )

    # Yesterday's completed weekly move becomes today's executable signal.
    ...
    for day in range(market.n_days):
        ...
        # Sell first so proceeds can fund the same day's buys. Never sell short.
        ...
        # If cash is constrained, divide it equally among today's buy signals.
        buy_assets = np.flatnonzero(buy_signal[day])
        if len(buy_assets):
            gross_each = min(
                buy_amount,
                cash / (len(buy_assets) * (1.0 + commission)),
            )
    ...
    benchmark = initial_cash * np.cumprod(
        1.0 + market.returns.mean(axis=1)
    )
